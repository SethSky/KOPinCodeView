...
-(void)initPinViewWithConfirmPIN:(BOOL)confirm
    ...
    if (confirm == YES) {
        ...
        if (!pinTitleLabel) {
            pinTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, yCenter, self.bounds.size.width, labelHeight)];
        ...
        if (!confirmPinTitleLabel) {
            confirmPinTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, yCenter + labelHeight + size.height, self.bounds.size.width, labelHeight)];
...
-(void)actions{
    ...
    if (string.length == array.count) {
        ...
        for (UIView *view in self.subviews) {
            ...
            if (view.tag == tf.tag) {
                [self animatioErrorCell:view andTextField:tf andSelect:YES];
            }
        }
    }
...
#pragma mark - SWPINCodeViewDelegate
